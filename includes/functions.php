...
function getRandomPlaylistLink($dir)
{
	...
	if ( $dir == $root || getFoldersCount($dir)>0 || getTracksCount($dir)>0 )
	{
		...
		$linkText = '<a href="'.$path.'" title="Lista aleatoria"><span class="glyphicon glyphicon-random pull-right breadcrumb"></span></a>';
		if ( !empty($debug) )
		{
...
function getBreadcrumbs($dir)
{
	...
	foreach ( $directories as $directory )
	{
		...
		$breadcrumbs .= '<li><a href="'.$path.'" title="Regresar">'.$directory.'</a>';
		...
		if ( $bcI == $count-1 && $playMode!='random' )
		{
			...
			if ( $count > 0 )
			{
				...
				$breadcrumbs .= '" title="Ejecutar todas las canciones"><span class="glyphicon glyphicon-play"></span></a>';
	...
	if ( $playMode == 'random' )
	{
		$breadcrumbs .= '<li>Lista Aleatoria</li>';
	}
	$breadcrumbs .= '</ol>';
	if ( !empty($debug) )
	{
...
function formatJsPlaylist($files)
{
	...
	if ( is_array($files) && count($files)>0 )
	{
		foreach ($files as $file)
		{
			...
			$trackLink = getTrackLink($dir,$track);
			$playlist .= '{file:"'.$trackLink.'",title: "'.$track.'",type:';
			$playlist .= '"'.strtolower(substr($track,-3)).'"},';
		}
		unset($files);
	}
		...
		foreach ($files as $file)
		{
			...
			switch($playMode)
			{
				...
				case "track":
					...
					$linkFront .= '" '.$class.' title="Solo reproducir esta canción">';
					break;
				case "random":
				case "folder":
					$linkFront = '<a href="#" '.$class.' onclick="jwplayer().playlistItem('.$i.');" title="Ir a esta canción">';
					break;
			}
			if ( $download )
			{
				...
				// Add download link
				$trackLink = getTrackLink($dir,$track);
				$getTracks .= '<a href="'.$trackLink.'&delivery=download" title="Descargar esta canción" target="_blank"><span class="glyphicon glyphicon-floppy-save"></span></a> &nbsp;<span id="databuscar-'.$i.'" data-track="'.$track.'" class="glyphicon glyphicon-search" title="Buscar acerca de esta canción en Google Search" onclick=buscar("databuscar-'.$i.'")></span> &nbsp;&middot;&nbsp; ';
			}
			$getTracks .= $linkFront;
